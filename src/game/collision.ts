...
export function colliders(area: RuntimeArea, region: Bounds, excludeIds?: ReadonlySet<string>): Collider[] {
  ...
  for (let y = Math.max(0, Math.floor(region.y / 16)); y <= Math.min(source.height - 1, Math.floor((region.y + region.height) / 16)); y++) {
    for (let x = Math.max(0, Math.floor(region.x / 16)); x <= Math.min(source.width - 1, Math.floor((region.x + region.width) / 16)); x++) {
      ...
      const id = `${source.id}:tile:${y}:${x}`;
      if (excludeIds?.has(id)) continue;
      result.push({ id, x: x * 16, y: y * 16, width: 16, height: 16,
        source: { kind: "tile", x, y, tile: { ...tile } }, ...(tile.kind === "hidden" ? { belowOnly: true } : {}) });
    }
  }
  for (const object of source.objects) {
    switch (object.kind) {
      case "pipe": case "billCannon": case "firebar":
  ...
  for (const body of area.platforms.bodies) {
  ...
  for (const collider of boundaries) if (!excludeIds?.has(collider.id)) result.push(collider);
  return result;
}
export function hasSolidOverlap(area: RuntimeArea, bounds: Bounds, excludeIds?: ReadonlySet<string>): boolean {
  return colliders(area, bounds, excludeIds).some(collider => !collider.belowOnly && overlaps(bounds, collider));
}
export function sweepAxis(area: RuntimeArea, bounds: Bounds, delta: number, axis: "x" | "y", excludeIds?: ReadonlySet<string>): { distance: number; contacts: Contact[] } {
  ...
  for (const collider of colliders(area, region, excludeIds)) {
