...
export function mountPlayGallery(root: HTMLElement): Readonly<{ dispose(): void }> {
  ...
  panel.innerHTML = `<h1>이동 실험실</h1><p>이동 ← → 또는 A D · 덩굴 ↑ · 달리기·불꽃 Shift 또는 X · 점프·수영 Space 또는 Z · 웅크리기 ↓ 또는 S · 일시정지 Esc</p><div class="movement-layout"><div></div><section><p>60 Hz 고정 시뮬레이션 · 화면 256×240픽셀<br>블록 · 아이템 · 지상 적 밟기 · 등껍질 연속 공격 · 불꽃 사용 가능<br>움직이는 발판 · 스프링 · 덩굴 타기 · 수중 수영 · 타이머 · 깃발/성 완료</p></section></div>`;
